...
const MyCourses = () => {
  // const { data: courseHistories, loading: courseHistoriesLoading } = useQuery(
  ...
  const courses = courseHistories?.courses || {};
  return (
    <>
      {/* Khoá học của tôi */}
      <div className="tab__content-item" style={{ display: "block" }}>
        <div className="courses__list">
          {!courseHistoriesLoading && courses?.length === 0 && (
            <Empty
              description="Không tồn tại khoá học nào"
              style={{ margin: "0 auto" }}
            />
          )}
          {courses?.length > 0 &&
            courses.map((item, index) => {
              return <CourseItem key={item.id || index} {...item} />;
            })}
